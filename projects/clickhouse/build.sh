...
cmake -GNinja $SRC/ClickHouse -DCMAKE_C_COMPILER=$CC -DCMAKE_CXX_COMPILER=$CXX -DENABLE_TESTS=1 -DENABLE_UTILS=0 \
        -DENABLE_RAPIDJSON=0 -DENABLE_JEMALLOC=0 -DSANITIZE=$SANITIZER -DENABLE_FUZZING=1 \
        -DLIB_FUZZING_ENGINE:STRING="$LIB_FUZZING_ENGINE" -DLINKER_NAME=lld -DENABLE_EMBEDDED_COMPILER=0 \
