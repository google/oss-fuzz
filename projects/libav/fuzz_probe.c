...
static int open_input_file(InputFile *ifile, const uint8_t* data, size_t size)
{
    int err, i;
    AVFormatContext *fmt_ctx = NULL;
    AVDictionaryEntry *t;
    const char *filename = "/tmp/fuzz.av";
    //write the input to a file
    FILE * fd;
    if (remove(filename) != 0) {
        if (errno != ENOENT) {
            fprintf(stderr, "failed remove, errno=%d\n", errno);
            return -1;
        }
    }
    fd = fopen(filename, "wb");
    if (fd == NULL) {
        fprintf(stderr, "failed open, errno=%d\n", errno);
        return -2;
    }
...
static int probe_file(const uint8_t* data, size_t size)
{
    InputFile ifile;
    int ret, i;

    ret = open_input_file(&ifile, data, size);
    if (ret < 0)
        return ret;
...
{
    fprintf(logfile, "%.*s", buf_size, buf);
    return 0;
}
