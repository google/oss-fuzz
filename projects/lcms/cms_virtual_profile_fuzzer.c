...
cmsHTRANSFORM hTransform = cmsCreateTransform(hInProfile, srcFormat, hOutProfile, TYPE_BGR_8, *((const uint32_t *)data+3) % 16,*((const uint32_t *)data+2));
...
cmsHPROFILE devicelinkProfile = cmsTransform2DeviceLink(hTransform, version, *((const uint32_t *)data+2));
...
cmsToneCurve* rgb_curves[15] = {tone,tone,tone,tone,tone,tone,tone,tone,tone,tone,tone,tone,tone,tone,tone};
cmsHPROFILE linearizationDeviceLinkProfile = cmsCreateLinearizationDeviceLink(srcCS, rgb_curves);
...
cmsHPROFILE limitingDeviceLinkProfile = cmsCreateInkLimitingDeviceLink(cmsSigCmykData, limit);
