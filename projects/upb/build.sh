...
sed -i 's#upb_deps()#upb_deps(skip_targets=["tcmalloc_and_profiler","tcmalloc_debug"])#' WORKSPACE
...
bazel build --verbose_failures --dynamic_mode=off --spawn_strategy=standalone \
  ...
  --define UPB_CONFIG_ASAN=1 --copt -D__SANITIZE_ADDRESS__ \
...
then
  ...
  rsync -av "${SRC}"/upb/source "${REMAP_PATH}"
  rsync -av "${SRC}"/upb/test "${REMAP_PATH}"
  # Clean up symlinks with a missing referrant.
  find "${SRC}"/upb/bazel-upb/external -follow -type l -ls -delete || echo "Symlink cleanup soft fail"
  rsync -avLk "${SRC}"/upb/bazel-upb/external "${REMAP_PATH}"
  ...
  rsync -avLk "${RSYNC_FILTER_ARGS[@]}" "${SRC}"/upb/bazel-out "${REMAP_PATH}"
