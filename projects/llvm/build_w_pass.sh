...
if [ -n "${OSS_FUZZ_CI-}" ]; then
  ...
  readonly FUZZERS=( \
    ...
    clang-format-fuzzer \
    clang-objc-fuzzer \
    # clangd-fuzzer \  # Reporter pass can't compile this fuzzer
    clang-pseudo-fuzzer \
    llvm-itanium-demangle-fuzzer \
...
export REPORT_FLAGS="-Xclang -load -Xclang /src/llvm/ReportFunctionExecutedPass/libReportPass.so -flegacy-pass-manager"
...
export CFLAGS="${CFLAGS:=} $REPORT_FLAGS /src/llvm/ReportFunctionExecutedPass/reporter.c++.o -lc++ -pthread -lm"
export CXXFLAGS="${CXXFLAGS:=} $REPORT_FLAGS /src/llvm/ReportFunctionExecutedPass/reporter.c++.o -pthread -lm"
export SILENT_REPORTER=1
