...
# If Makefile is missing, we try to run configure.
# Note: we assume autogen.sh was already run during the initial build
# and all gnulib files are already present in the container.
if [ ! -f Makefile ]; then
