      ...
      switch (input_event.type)
      {
          case YAML_STREAM_START_EVENT:
              ...
              if (!yaml_document_append_mapping_pair(&output_document,
                          properties, key, value)) goto error;
              ...
              if (input_event.data.stream_start.encoding)
              {
                  ...
                  if (!key) goto error;
                  value = yaml_document_add_scalar(&output_document, NULL,
                          (encoding == YAML_UTF8_ENCODING ? (yaml_char_t *)"utf-8" :
                           encoding == YAML_UTF16LE_ENCODING ? (yaml_char_t *)"utf-16-le" :
                           encoding == YAML_UTF16BE_ENCODING ? (yaml_char_t *)"utf-16-be" :
                           (yaml_char_t *)"unknown"), -1, YAML_PLAIN_SCALAR_STYLE);
                  if (!value) goto error;
                  if (!yaml_document_append_mapping_pair(&output_document,
                              properties, key, value)) goto error;
              }
          ...
          case YAML_DOCUMENT_START_EVENT:
              ...
              value = yaml_document_add_scalar(&output_document, (yaml_char_t *)YAML_BOOL_TAG,
                      (input_event.data.document_start.implicit ?
                       (yaml_char_t *)"true" : (yaml_char_t *)"false"), -1, YAML_PLAIN_SCALAR_STYLE);
              if (!value) goto error;
              if (!yaml_document_append_mapping_pair(&output_document,
                          properties, key, value)) goto error;
