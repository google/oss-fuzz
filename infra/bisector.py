...
def main():
  ...
  try:
    ...
    return 1
  rm = DockerRepoManager(repo_name)
  commit_list = rm.get_commit_list(args.commit_old, args.commit_new)
  result_commit_idx = bisection(0, len(commit_list) - 1, commit_list, rm, -1, args.test_case, args.fuzzer_name)
...
def bisection(commit_old_idx, commit_new_idx, commit_list, repo_manager, last_error, test_case, fuzzer_name):
  ...
  if commit_new_idx == commit_old_idx:
    if error_exists:
      ...
      return last_error
  if error_exists:
    return bisection(commit_old_idx, cur_idx - 1, commit_list, repo_manager, cur_idx, test_case, fuzzer_name)
  else:
    return bisection(cur_idx + 1, commit_new_idx, commit_list, repo_manager, last_error, test_case, fuzzer_name)
