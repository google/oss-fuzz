...
void InMemoryIndex::Merge(const InMemoryIndex& other) {
  ...
  for (const auto& [location, id] : other.locations_) {
    new_location_ids[id] = GetPreparedLocationId(location);
  }
...
LocationId InMemoryIndex::GetLocationId(Location location) {
  ...
  return GetPreparedLocationId(location);
}

LocationId InMemoryIndex::GetPreparedLocationId(const Location& location) {
