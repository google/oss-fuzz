...
const clang::CXXRecordDecl* GetRecordForType(const clang::QualType& type) {
  ...
  CHECK(record_type);
  const clang::Decl* decl = record_type->getDecl();
  CHECK(decl);
  ...
  if (llvm::isa<clang::CXXDeleteExpr>(expr)) {
    ...
    if (clang::Expr* base = member_expr->getBase()) {
      ...
      if (!member_expr->getQualifierLoc()) {
        if (const auto* expr_record_decl = GetRecordForType(
              base->IgnoreParenBaseCasts()->getType())) {
...
void AstVisitor::AddSyntheticMemberReference(
  ...
  const Entity synthetic_inherited_member =
      Entity(base_member_entity, GetNamePrefixForDeclContext(child_class),
