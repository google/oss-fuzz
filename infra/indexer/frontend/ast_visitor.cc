...
const clang::NamedDecl* GetTemplatePrototypeNamedDecl(const clang::Decl* decl) {
  ...
  const clang::DeclContext* template_context;
  ...
  if (const auto* class_specialization_decl =
    ...
    if (const clang::FunctionDecl* instantiation_pattern =
    ...
    } else if (const clang::FunctionDecl* member_function =
                   function_decl->getInstantiatedFromMemberFunction()) {
      template_context = member_function;
      template_decl = member_function->getDescribedFunctionTemplate();
      ...
      template_decl = function_decl->getDescribedFunctionTemplate()
                          ->getInstantiatedFromMemberTemplate();
      template_context =
          llvm::cast<clang::CXXMethodDecl>(template_decl->getTemplatedDecl());
    } else {
      return nullptr;
    }
  } else {
...
std::string GetNamePrefix(const clang::Decl* decl) {
  ...
  return GetNamePrefixForDeclContext(decl_context, include_function_scope);
}
...
void ForAllInheritedMembers(clang::Sema& sema,
  ...
  for (const clang::DeclarationName& decl_name : seen_names) {
    ...
    for (const auto decl : lookup_result) {
      // Check it is an inherited member and not one from the class itself.
...
bool AstVisitor::VisitRecordDecl(clang::RecordDecl* decl) {
  if (auto* record_decl = llvm::dyn_cast<clang::CXXRecordDecl>(decl)) {
    ...
    if (IsADefinition(record_decl)) {
      SynthesizeInheritedFieldEntities(record_decl);
    }
...
void AstVisitor::SynthesizeInheritedFieldEntities(
    const clang::CXXRecordDecl* class_decl) {
  CHECK(IsADefinition(class_decl));

  const std::string new_name_prefix = GetNamePrefixForDeclContext(class_decl);
...
std::optional<Entity> AstVisitor::GetEntityForDecl(const clang::Decl* decl,
  ...
  const std::string name_prefix = GetNamePrefix(decl);
  const std::string name_suffix = GetNameSuffix(decl);
  ...
  if (llvm::isa<clang::VarDecl>(decl) || llvm::isa<clang::FieldDecl>(decl) ||
    ...
    } else if (function_decl->getInstantiatedFromMemberFunction()) {
      function_template = function_decl->getInstantiatedFromMemberFunction();
    } else if (function_decl->getDescribedFunctionTemplate() &&
               function_decl->getDescribedFunctionTemplate()
                   ->getInstantiatedFromMemberTemplate()) {
