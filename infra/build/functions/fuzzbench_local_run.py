...
LOG_FILE_PATH = os.path.join(os.path.dirname(__file__), 'fuzzbench_local_run_log.txt')
...
def run_step_locally(temp_dir, local_fuzzbench_path, step, i, log_file):
    ...
    try:
        ...
        log_file.write(f'--- Step {i} completed successfully --- Took {end_time - start_time}s\n\n')
    except subprocess.CalledProcessError as e:
        if e.returncode == 124:
          end_time = time.time()
          log_file.write(f'Caught timeout: {e}\n')
          log_file.write(f'--- Step {i} completed with a timeout --- Took {end_time - start_time}s\n\n')
        else:
...
def main():
  ...
  if not fuzz_target_name:
    ...
    if not fuzz_target_name:
      raise Exception('Fuzz Introspector Web API did not find a fuzz target. Provide one')
  ...
  steps += fuzzbench.get_build_fuzzers_steps(config.fuzzing_engine, project, env)
  env_dict = {string.split('=')[0]: string.split('=')[1] for string in env}
  steps += fuzzbench.get_gcs_corpus_steps(config.fuzzing_engine, project, env_dict)
  steps += fuzzbench.get_build_ood_image_steps(config.fuzzing_engine, project, env_dict)
  steps += fuzzbench.get_push_and_run_ood_image_steps(config.fuzzing_engine, project,
                                            env_dict)
