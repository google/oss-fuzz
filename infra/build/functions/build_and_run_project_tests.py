...
#
################################################################################
"Functions for building and running project tests."
import argparse
import collections
import datetime
import functools
...
def get_projects_to_build(specified_projects, build_type, force_build):
  ...
  for project in specified_projects:
    if (
        project not in project_statuses or project_statuses[project] or
        force_build):
...
def _do_build_type_builds(args, config, credentials, build_type, projects):
  ...
  for project_name in projects:
    ...
    try:
      tags = ['trial-build', f'branch-{args.branch.lower().replace("/", "-")}']
      if args.version_tag:
        tags.append(f'version-{args.version_tag}')

      build_ids[project_name] = (
          build_project.run_build(project_name,
                                  steps,
                                  credentials,
...
def gcb_build_and_run_project_tests(args, test_image_suffix, end_time):
...
def main():
  ...
  return 0 if gcb_build_and_run_project_tests(args, test_image_suffix, end_time) else 1
