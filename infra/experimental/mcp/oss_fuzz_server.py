...
async def check_if_oss_fuzz_project_builds(project_name: str) -> bool:
    ...
    try:
        ...
        logger.info("Build failed for project '%s': {%s}", project_name,
                    str(e))
        return False
...
async def get_sample_artifacts_from_oss_fuzz_project(
    ...
    """
    logger.info(
        "Retrieving sample artifacts for OSS-Fuzz project language: %s",
        language)
    if language == 'go':
        project_name = 'go-dns'
    elif language == 'c':
...
async def check_oss_fuzz_fuzzers(
    ...
    try:
        subprocess.check_call('python3 infra/helper.py check_build ' +
                              project_name,
                              cwd=oss_fuzz_mcp_config.BASE_OSS_FUZZ_DIR,
                              shell=True,
                              stdout=log_stdout,
        ...
        logger.info("Check failed for project '%s': {%s}", project_name,
                    str(e))
