...
def set_up_environment(work_dir):
  ...
  wrapper_utils.install_wrapper(bin_dir, 'dpkg-gensymbols', gen_symbols_wrapper)

  # Install no-op strip binaries.
  no_op_strip = ('#!/bin/sh\n' \
                 'exit 0\n')
  wrapper_utils.install_wrapper(bin_dir, 'strip', no_op_strip,
  ...
  make_wrapper = ('#!/bin/bash\n'
                  ...
                  'fi\n'
                  '/usr/bin/make "$@"\n')
  wrapper_utils.install_wrapper(bin_dir, 'make', make_wrapper)
...
def _collect_dependencies(apt_cache, pkg, cache, dependencies):
  ...
  for dependency in pkg.candidate.dependencies:
    ...
    if dependency.name in cache:
      ...
      is_c_or_cxx |= _collect_dependencies(apt_cache,
                                           apt_cache[dependency.name], cache,
                                           dependencies)
  if is_c_or_cxx:
